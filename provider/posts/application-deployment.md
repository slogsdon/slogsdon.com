...
tags: elixir, deployment, exrm, functional-programming
---
...
> __Psst!__ Your hosting environment and your build environment can be one in the same if you're doing a test run. 
...
To get started, we'll need to add exrm into our list of dependencies. With later versions of Elixir, we just need to add `{ :exrm, "~> 0.14.7" }` to have [hex](https://hex.pm) download everything correctly.  Here's an example:
...
> __Warning!__ The `phoenix.start` mix task will stop working once that worker is in your application's supervisor tree because `MyApp.Router.start` would be started twice, resulting in a failure from the second operation. To easily bring up a development server, run `iex -S mix`, which will give you a running application and an `iex` console as well.

### Generating the release

Running `mix release` will kick off the build process for our release.

> __Note:__ In the following sections, you'll see our application's version (`0.0.1`) pop up in bunch of places. This value is pulled from the application's `mix.exs` file, under the project's version.
...
> __Note:__ These points hold true for Apache as well, but the steps to accomplish them are slightly different.
