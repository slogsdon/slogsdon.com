...
${(
  <Image
    src="/static/images/asp-dotnet-core-webapi-in-azure-app-service/create-new-resource.png"
    height="337px"
  />
)}
...
${(
  <Image
    src="/static/images/asp-dotnet-core-webapi-in-azure-app-service/app-deployment.png"
    height="650px"
  />
)}
...
After taking a break, I decided to use my brain and found this [StackOverflow answer](http://stackoverflow.com/a/40658723/771757) which was the key to correcting the ${(
  <InlineCode>{`MSB4025`}</InlineCode>
)} issue. The fix is to add a ${(
  <InlineCode>{`global.json`}</InlineCode>
)} file to your project with a specification with the specific version of the SDK being used locally.

If generating a project with ${<InlineCode>{`dotnet new`}</InlineCode>} or ${(
  <InlineCode>{`yo aspnet`}</InlineCode>
)}, you'll need to move your project around as well to accomodate the ${(
  <InlineCode>{`global.json`}</InlineCode>
)} file's way of specifying projects:
...
)} (or whatever you choose to name it). From there, we need to add some content to ${(
  <InlineCode>{`global.json`}</InlineCode>
)}:
...
_Update 03 February 2017_: Moving your project files to ${(
  <InlineCode>{`src`}</InlineCode>
)} does not seem to be a hard requirement. Dan Clarke pointed this out to me [via Twitter](https://twitter.com/dracan/status/827406653754531840), stating the following ${(
  <InlineCode>{`global.json`}</InlineCode>
...
This would have the benefit of not requiring a change in project structure, but if you like to keep your source and test projects separate (I do), you can keep your main project in ${(
  <InlineCode>{`src`}</InlineCode>
)} and a test project in ${<InlineCode>{`test`}</InlineCode>}, ensuring that ${(
  <InlineCode>{`test`}</InlineCode>
)} was added to the ${<InlineCode>{`projects`}</InlineCode>} array in the ${(
  <InlineCode>{`global.json`}</InlineCode>
)} file. Either way should get the job done at the end of the day, so be sure to pick what makes most sense to you and your project.

If, like me, you have no idea which SDK your locally installed ${(
  <InlineCode>{`dotnet`}</InlineCode>
)} is using, running ${(
  <InlineCode>{`dotnet --version`}</InlineCode>
)} will give the exact string needed under ${(
  <InlineCode>{`sdk.version`}</InlineCode>
)} in your config. Once set up, you can commit all of those changes and push them up to your app, eventually seeing the below:
...
${(
  <Image
    src="/static/images/asp-dotnet-core-webapi-in-azure-app-service/visit-project.png"
    height="430px"
  />
)}
...
)} away thanks to Azure App Service!
`);
