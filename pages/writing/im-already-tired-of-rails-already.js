...
export default withMd({
  ...
  date: "2013-08-15",
  tags: ["acvte", "golang", "obtvse2", "ruby"],
})(markdown(components)`
...
As a career developer, I'm always messing with the latest and greatest tools and languages that interest me. Coming across [Revel][4], a web application framework for Go, I decided to take it for a test run. After updating my go environment to make use of the newly released ${(
  <InlineCode>{`go1.1.2`}</InlineCode>
)}, I easily had a revel app up and running.

After the initial compile after starting the app, something I've grown accustom to from deploying .NET apps at work, response times reported by Chrome Inspector are sitting at about 6ms locally. Right on par with Phalcon. How about memory consumption? Using ${(
  <InlineCode>{`revel run revel-test`}</InlineCode>
)} to run it, the app is using 7MB with ${(
  <InlineCode>{`revel`}</InlineCode>
)} itself using 10MB. I'm assuming this will decrease after building for production, but even at 17MB, the Revel app is coming in as the lightweight champ.
