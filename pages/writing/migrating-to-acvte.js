...
export default withMd({
  ...
  date: "2013-08-31",
  tags: ["acvte", "devops", "golang", "rails"],
})(markdown(components)`
...
The table above details the actual memory usage of both apps. Obtvse2, well rails actually, was using 113MB, an average amount across the app's lifespan. Acvte, on the other hand, was only using 8MB. Here's a fun fact. Three instances of acvte (${(
  <InlineCode>{`blog`}</InlineCode>
)}, ${<InlineCode>{`acvte`}</InlineCode>}, and ${(
  <InlineCode>{`bash`}</InlineCode>
)} in the table) combined used only ~21% of the memory used by the rails app. The ${(
  <InlineCode>{`bash`}</InlineCode>
)} instance was me running acvte in dev mode for some debugging, while the other two were running via ${(
