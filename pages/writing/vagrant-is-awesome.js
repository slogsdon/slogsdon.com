...
export default withMd({
  ...
  date: "2013-08-07",
  tags: ["devops", "phalcon", "php", "vagrant"],
})(markdown(components)`
...
The only downside of using Phalcon is having to compile and load it for use by PHP. ${(
  <InlineCode>{`git clone`}</InlineCode>
)}-ing the [cphalcon repo][5] and running the build script is easy enough, but what if you're getting help from a creative that has no idea how to set up a virtual host on their local machine, let alone compiling a PHP extension?

## Use Vagrant, of course!

I've been using Vagrant to host a Phalcon project for a while, and I managed to extract out the Vagrant goodies into a stand-alone project: [vagrant-phalcon][6]. With the help of a Puppet manifest and a bash script, vagrant-phalcon helps developers of all skill levels start with Phalcon with an easy ${(
  <InlineCode>{`vagrant up`}</InlineCode>
...
> 3. Clone this repo
> 4. Run ${<InlineCode>{`vagrant up`}</InlineCode>} in the repo directory
> 5. Visit ${(
  <InlineCode>{`http://localhost:8081/`}</InlineCode>
)} in your favorite browser
>
> ##### Note
>
> Nginx is configured by default to use ${(
  <InlineCode>{`./src/Public/`}</InlineCode>
)} as the root directory. If your public directory differs, be sure to update the ${(
  <InlineCode>{`Vagrantfile`}</InlineCode>
