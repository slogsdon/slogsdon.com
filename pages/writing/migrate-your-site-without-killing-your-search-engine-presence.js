...
export default withMd({
  ...
  date: "2013-12-20",
  tags: ["apache", "devops", "migration", "nginx", "redirects"],
...
If you're using a control panel like CPanel, Plesk, etc., you may have the option to set up your redirects through the control panel. Beware that some, like Plesk, use temporary (302) redirects which are not desired. In those cases, you will want to set up your redirect rules manually with a ${(
  <InlineCode>{`.htaccess`}</InlineCode>
)} file. Hosts with control panels typically use Apache for the web server, so ${(
  <InlineCode>{`.htaccess`}</InlineCode>
)} files, which are Apache-specific, will work just fine. If nginx is used, a ${(
  <InlineCode>{`.htaccess`}</InlineCode>
)} file will accomplish nothing, so redirect will need to be set up in the virtual host config file for the domain.
...
If you're using GitHub Pages, bad news bears for you. Redirects are not yet supported by the web server, so you're reduced down to using Javascript or a meta refresh if you want your website to handle the redirect. Doing either of those, you will want to add a ${(
  <InlineCode
  >{`<link rel="canonical" href="http://www.yourdomain.com/...">`}</InlineCode>
)} to prevent duplicate content as the redirects are only issued as temporary (302).
...
These can be placed in your virtual host config file in a ${(
  <InlineCode>{`<Directory>`}</InlineCode>
)} node or in a ${<InlineCode>{`.htaccess`}</InlineCode>} file:
...
These will need to be placed in a ${(
...
For frameworks and some languages, there will typically be a method somewhere to allow for the redirect, otherwise, you will need to set the HTTP status code and ${(
  <InlineCode>{`Location`}</InlineCode>
)} header with the new location manually as long as you haven't started sending a response to the end-user. Router configurations are a great place for setting redirect rules. If your using a MVC framework, your controller actions could be another option.
...
I ran into some luck with my redirects since I was moving to GitHub pages where redirects cannot be done with the web server. Because I was moving to a different domain as well, I left my old domain pointed to my cloud server, setting up my redirects in my nginx ${(
  <InlineCode>{`shanelogsdon.com`}</InlineCode>
)} server block.
