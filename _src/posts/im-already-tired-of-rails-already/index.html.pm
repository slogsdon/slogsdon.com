#lang pollen
◊define-meta[title]{I’m already tired of Rails...already}
◊define-meta[author]{shane}
◊define-meta[publish-date]{2013-08-15}
◊define-meta[tags]{acvte, golang, obtvse2, ruby}
...
When I first launched this site, I was publishing with ◊a[#:href "http://scriptogr.am/"]{Scriptogr.am} but soon decided to self-host. In a weekend, I custom-rolled a solution similar to Scriptogr.am, with markdown files being parsed for the same information used by Scriptogr.am, written in C# with MVC4. It was fast enough, and Mono was only using ~80MB. Not perfect but acceptable.

Fast-forward a couple months. I've always wanted liked the ◊a[#:href "https://github.com/natew/obtvse2"]{obtvse} project, so I decided to take the Rails app for a spin as my production site. Migration was simple once I figured out the proper steps needed to host it behind nginx. Having already set up a reverse-proxy cache, my page load times are nothing to shake a stick at, but NewRelic reports my app response times to be 20-30ms slower than a ◊a[#:href "http://phalconphp.com/"]{Phalcon}-based app I'm currently developing, which consistently has response-times lower than 6ms.
...
◊h2{golang to the rescue?}

As a career developer, I'm always messing with the latest and greatest tools and languages that interest me. Coming across ◊a[#:href "http://robfig.github.io/revel/"]{Revel}, a web application framework for Go, I decided to take it for a test run. After updating my go environment to make use of the newly released ◊code{go1.1.2}, I easily had a revel app up and running.

After the initial compile after starting the app, something I've grown accustom to from deploying .NET apps at work, response times reported by Chrome Inspector are sitting at about 6ms locally. Right on par with Phalcon. How about memory consumption? Using ◊code{revel run revel-test} to run it, the app is using 7MB with ◊code{revel} itself using 10MB. I'm assuming this will decrease after building for production, but even at 17MB, the Revel app is coming in as the lightweight champ.
...
I'm already excited, and I haven't even started. I wonder if I can write a lean GitHub clone that doesn't use upwards of 500MB of my server's RAM. Yeah, I'm looking at you, ◊a[#:href "https://github.com/gitlabhq/gitlabhq"]{GitLab}. I like you, but that's stupid.
