#lang pollen
◊define-meta[title]{Migrate Your Site Without Killing Your Search Engine Presence}
◊define-meta[author]{shane}
◊define-meta[publish-date]{2013-12-20}
◊define-meta[tags]{apache, devops, migration, nginx, redirects}
...
◊h3{Determine new URL structure}
...
◊h3{How are the redirects going to be handled?}
...
◊h4{Letting your web server handle it}

Web server based redirects will be your best bet. They will help reduce latency in the request by redirecting it before your application has a chance to touch it. Seeing that ◊a[#:href "http://httpd.apache.org/"]{Apache} and ◊a[#:href "http://nginx.org/"]{nginx}, there will be examples of ways to set up the redirects in both later in this section. In any case, we want to use permanent (301) redirects to let browsers and search engine bots know that the old resource will never be used again and to remember the new URL.

If you're using a control panel like CPanel, Plesk, etc., you may have the option to set up your redirects through the control panel. Beware that some, like Plesk, use temporary (302) redirects which are not desired. In those cases, you will want to set up your redirect rules manually with a ◊code{.htaccess} file. Hosts with control panels typically use Apache for the web server, so ◊code{.htaccess} files, which are Apache-specific, will work just fine. If nginx is used, a ◊code{.htaccess} file will accomplish nothing, so redirect will need to be set up in the virtual host config file for the domain.

◊h5{Side note for static sites on ◊a[#:href "https://pages.github.com/"]{GitHub Pages} or ◊a[#:href "http://aws.amazon.com/s3/"]{S3}}

Since static site generators and hosting on GitHub or S3 are all the rage right now, I should make a couple notes.

If you're using S3, congratulations! Follow the ◊a[#:href "http://docs.aws.amazon.com/AmazonS3/latest/dev/how-to-page-redirect.html#page-redirect-using-console"]{AWS docs}, and set up your redirects in the AWS S3 console.
...
These can be placed in your virtual host config file in a ◊code{<Directory>} node or in a ◊code{.htaccess} file:
...
These will need to be placed in a ◊code{server} configuration block:
...
◊h4{Your application is next up}
...
For applications similar to WordPress with large plugin/extension libraries, look for one that will handle the redirects for you. WordPress, as an example, has the ◊a[#:href "http://wordpress.org/plugins/redirection/"]{Redirection} plugin.

For frameworks and some languages, there will typically be a method somewhere to allow for the redirect, otherwise, you will need to set the HTTP status code and ◊code{Location} header with the new location manually as long as you haven't started sending a response to the end-user. Router configurations are a great place for setting redirect rules. If your using a MVC framework, your controller actions could be another option.
...
◊h3{Let the search engines know}
...
I ran into some luck with my redirects since I was moving to GitHub pages where redirects cannot be done with the web server. Because I was moving to a different domain as well, I left my old domain pointed to my cloud server, setting up my redirects in my nginx ◊code{shanelogsdon.com} server block.

◊h3{Updating my sitemap}
